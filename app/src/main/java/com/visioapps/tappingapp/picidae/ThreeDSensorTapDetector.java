...
public class ThreeDSensorTapDetector {
   ...
   */
  public ThreeDSensorTapDetector(TapListener tapListener, float sensorMaxScale, ThreeDSensorTapDetectorType type) {
  ...
  public void onSensorChanged(long timestamp, float[] values) {
    ...
    if (DEBUG) {
      Log.v(
          ...
          String.format("State %s, CurrentEnergy %f, size %d, limit %f, signal %f", mCurrentState.name(), mConditionedSignalEnergy,
              mEnergySamplesList.size(), mEnergySamplesList.size() * mDetectorType.energyPerSampleNoiseLimit, mLastConditionedMagnitudeSq));
    }
  ...
  private void stateMachineProcessingDefiniteTap(long timestamp) {
    ...
    if (mLastConditionedMagnitudeSq > envelope) {
      ...
      if (DEBUG) {
        Log.v("threeDSensorTapDetector", String.format("Tap downgraded to possible at %d. Signal %f limit %f",
  ...
  private void stateMachineProcessingPossibleTap(long timestamp) {
    ...
    if (mLastConditionedMagnitudeSq > envelope) {
      if (DEBUG) {
        Log.v("threeDSensorTapDetector", String.format("Tap downgraded to noise at %d. Signal %f limit %f",
                timestamp, mLastConditionedMagnitudeSq, envelope));
      }
  ...
  private void changeToNewCurrentState(long timestamp, SensorDetectorState newState) {
    ...
    if (DEBUG) {
      Log.v("threeDSensorTapDetector", String.format("Transition to State %s at time %d", mCurrentState.name(), timestamp));
    }
