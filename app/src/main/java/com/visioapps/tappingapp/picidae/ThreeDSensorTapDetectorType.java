...
/**
 * Specific types of tap detectors for accelerometer and gyroscope that initialize
 * ThreeDSensorTapDetector. Encapsulates the large number of parameters needed to support different
 * sensor types.
 */
@SuppressWarnings({
  "PointlessArithmeticExpression", // Allow "0 * XXXX" so that units are clear.
  "ImmutableEnumChecker" // Allow filterNum and filterDen to be arrays.
})
public enum ThreeDSensorTapDetectorType {
  ...
  /**
   * The three axes are all filtered with an IIR filter. This filter must be be first order, so both
   * filterNum and filterDen have length 2. filterDen[0] is ignored and assumed to be 1.0f.
   */
  ...
  /**
   * Start processing a possible tap if we're above this multiple of current noise during NOISY
   * state
   */
  ...
  /**
   * Once we've starting detecting a tap, we want to see its energy stay inside an envelope. The
   * envelope is infinite for the HighAmplitudeTime, and then drops to the maximum detected energy
   * and then falls linearly to the LowLevel during the FallTime. It then stays at LowLevel for
   * LowTime.
   */
   ...
   * @param energyPerSampleNoiseLimitInit Limit for detecting noise. Sensor units.
   * @param multipleOfNoiseForPossibleTapInit Threshold to detect possible tap when signal is noisy
   * @param definiteTapsHighAmplitudeTimeNanosInit Time definite taps may be high amplitude
   * @param possibleTapsHighAmplitudeTimeNanosInit Time possible taps may be high amplitude
   * @param definiteTapsFallTimeNanosInit Time definite taps must drop from high to low amplitude
   * @param possibleTapsFallTimeNanosInit Time possible taps must drop from high to low amplitude
   ...
   * @param definiteTapsLowTimeNanosInit Time definite taps must stay at low level
   * @param possibleTapsLowTimeNanosInit Time possible taps must stay at low level
   */
  private ThreeDSensorTapDetectorType(
      float[] filterNumInit,
      float[] filterDenInit,
      float energyPerSampleNoiseLimitInit,
      float multipleOfNoiseForPossibleTapInit,
      long definiteTapsHighAmplitudeTimeNanosInit,
      long possibleTapsHighAmplitudeTimeNanosInit,
      long definiteTapsFallTimeNanosInit,
      long possibleTapsFallTimeNanosInit,
      float definiteTapsLowLevelInit,
      float possibleTapsLowLevelInit,
      long definiteTapsLowTimeNanosInit,
    ...
    thresholdForPossibleTap = energyPerSampleNoiseLimitInit * multipleOfNoiseForPossibleTapInit;
