...
package com.visioapps.tappingapp.picidae;
 ...
 *
 * <p>Raw data can be saved to a LittleEndianDataOutputStream for development.
 *
 * <p>This detector integrates output from individual detectors that each detect taps based on a
 * single sensor. Options exist to consider only one sensor at a time and to qualify taps based on a
 * tap quality, which can range from 0.0 to 1.0.
 *
 * <p>Functions adjust the minimum spacing between two individual taps, as well as the maximum
 * spacing between the taps of a double tap. These two parameters are orthogonal, as a double tap is
...
public class IntegratedTapDetector implements SensorEventListener, ThreeDSensorTapDetector.TapListener {
  ...
  /**
   * Guidance value for setTapDetectionQuality. Try not to lose any taps and accept that there will
   * be quite a few false positives. This value is the default for double tap quality, as the first
   * tap qualifies the second one.
   */
  public static final double TAP_QUALITY_LOW = TAP_QUALITY_POSSIBLE;

  /**
   * Guidance value for setTapDetectionQuality. Balance between false positives and false negatives.
   */
  public static final double TAP_QUALITY_MEDIUM = 2 * TAP_QUALITY_POSSIBLE;

  /**
   * Guidance value for setTapDetectionQuality. Work hard to avoid false positives. This is the
   * default for tap detection.
   */
  ...
  // Visible for testing
  public IntegratedTapDetector(SensorManager sensorManager, ThreeDSensorTapDetector accelTapDetector,
    ...
    if (accelTapDetector == null) {
      ...
      accelTapDetector = new ThreeDSensorTapDetector(this, maxRange, ThreeDSensorTapDetectorType.ACCELEROMETER);
    }
    ...
    if (gyroTapDetector == null) {
      ...
      gyroTapDetector = new ThreeDSensorTapDetector(this, maxRange, ThreeDSensorTapDetectorType.GYROSCOPE);
    }
  ...
  /**
   * Choose the maximum spacing in nanos for double-taps, which are reported as separate events from
   * taps. Default is 0, which disables double-tap detection.
   *
   * @param maxDTapSpacing taps closer together than this will be combined into a single double tap.
   *     0 disabled double-tap detection.
   */
  ...
  /**
   * Set desired double-tap quality. Double-taps must have at least one tap that is of standard tap
   * quality, and one over this quality.
   *
   * @param tq value between 0 and 1. Larger numbers are higher quality taps.
   */
  ...
  public void start() {
    Sensor accelerometer = mSensorManager.getDefaultSensor(Sensor.TYPE_ACCELEROMETER);
    mSensorManager.registerListener(this, accelerometer, SensorManager.SENSOR_DELAY_FASTEST, mHandler);

    Sensor gyroscope = mSensorManager.getDefaultSensor(Sensor.TYPE_GYROSCOPE);
    mSensorManager.registerListener(this, gyroscope, SensorManager.SENSOR_DELAY_FASTEST, mHandler);
  }
  ...
  @Override
  public void threeDSensorTapDetected(ThreeDSensorTapDetector threeDSensorTapDetector, long timestamp, double tapConfidence) {
  ...
  private void integrateAccelAndGyroQueues(long timestamp) {
    ...
    while ((mGyroTapEventQueue.size() > 0) && (mAccelTapEventQueue.size() > 0)) {
      if (mGyroTapEventQueue.peek().nanos > mAccelTapEventQueue.peek().nanos + MAX_OFFSET_BETWEEN_TAP_DETECTORS) {
        mIntegratedTapEventQueue.add(mAccelTapEventQueue.remove());
        continue;
      }

      if (mAccelTapEventQueue.peek().nanos > mGyroTapEventQueue.peek().nanos + MAX_OFFSET_BETWEEN_TAP_DETECTORS) {
      ...
      Tap integratedTap = new Tap(accelTap.quality + gyroTap.quality, Math.min(accelTap.nanos, gyroTap.nanos));
     ...
     */
    Queue<Tap> nonEmptyTapQueue = (mGyroTapEventQueue.size() > 0) ? mGyroTapEventQueue : mAccelTapEventQueue;
    while (nonEmptyTapQueue.size() > 0) {
      long latestTimeThatCantBeADoubleTap = timestamp - MAX_OFFSET_BETWEEN_TAP_DETECTORS - MAX_TAP_DETECTOR_LATENCY;
      if (nonEmptyTapQueue.peek().nanos > latestTimeThatCantBeADoubleTap) {
        break;
      }

      if (DEBUG) {
        Log.v("Picidae", String.format("Adding tap at time %d from nonempty to integrated queue at time %d",
  ...
  private void processIntegratedQueueAsSingleAndDoubleTaps(long timestamp) {
    while (mIntegratedTapEventQueue.size() >= 2) {
      ...
      if (!tapAllowedAt(olderTap.nanos)) {
        if (DEBUG) {
          Log.v("Picidae", String.format("Disallowing tap at time %d with quality %f", olderTap.nanos, olderTap.quality));
        }
        continue;
      }
      ...
      if (newerTap.nanos < olderTap.nanos + mMaxDoubleTapSpacingNanos) {
         ...
         */
        boolean qualityGoodEnough1 =
            (newerTap.quality >= mMinTapQuality) && (olderTap.quality >= mMinDoubleTapQuality);
        boolean qualityGoodEnough2 =
      ...
      if (olderTap.quality >= mMinTapQuality) {
        ...
        Log.v("Picidae", String.format("Discarding tap at time %d with quality %f", olderTap.nanos, olderTap.quality));
      }
    }
    ...
    long maxOffsetToConsider = (mMaxDoubleTapSpacingNanos > 0) ? MAX_OFFSET_BETWEEN_TAP_DETECTORS + MAX_TAP_DETECTOR_LATENCY : 0;
    if (mIntegratedTapEventQueue.peek().nanos
      ...
      if ((tap.quality >= mMinTapQuality) && tapAllowedAt(tap.nanos)) {
        ...
        Log.v(
            "Picidae", String.format("Discarding tap at time %d with quality %f", tap.nanos, tap.quality));
      }
    }
  }
  ...
  /**
   * List of different tap detection quality required to report a tap. Higher qualities have lower
   * false positive rates but are more likely to miss taps. Some detectors - OTP, for example, are
